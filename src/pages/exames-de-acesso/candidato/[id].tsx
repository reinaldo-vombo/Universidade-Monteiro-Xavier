...
import { Field, PagamentoRow, SectionCard } from '../../../components/candite-items';
...
export default function SigleRegisters(params: any) {
   const exameId = params.id

   const { data: registo, isLoading } = useQuery({
      queryKey: ['registo', exameId],
      queryFn: () => api.exames.byExameId(exameId),
      staleTime: 1000 * 60 * 2, // 2 min — estado de pagamento muda mais
   })

   if (!registo) {
      return 'Cabdidate not found'
   }
   if (isLoading) {
      return 'loading'
   }
   ...
   return (
      <main className="min-h-screen">
         {/* Hero / Header */}
         <div className="bg-[#0D0D0D] md:h-1/2 px-4 md:px-20 pt-8 md:pt-36 pb-16">
            <Link
               to="/exames-de-acesso"
         ...
         <SEO title={`${registo.firstName} ${registo.lastName} — Registo de Exame`} />
      </main>
